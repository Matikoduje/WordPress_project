...
class Merlin {
	...
	/**
	 * The title and url of the features array.
	 *
	 * @var array $features
	 */
	 ...
	 * @param array $features Title and URL for the features.
	 */
	function __construct( $config = array(), $strings = array(), $features = array() ) {
		...
		// Features array from the config file.
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'blockst' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'blockst' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'blockst' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'blockst' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'blockst' ),
				'view' => array( $this, 'plugins' ),
			);
		}

		// Show the pro version features, only for the free users.
		if ( blockst_fs()->is_not_paying() ) {
			$this->steps['pro-version'] = array(
				'name' => esc_html__( 'Pro Version', 'blockst' ),
				'view' => array( $this, 'pro_version' ),
			);
		}
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'blockst' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'blockst' ),
	...
	* Pro Version
	*/
	protected function pro_version() {
		// Theme Name.
		...
		// Strings passed in from the config file.
		$strings = $this->strings;
		// Features array passed in from the config file.
		$features = $this->features;
		// Text strings.
		$header    = $strings['pro-version-header%s'];
		$skip      = $strings['btn-skip'];
		$next      = $strings['btn-next'];
		$paragraph = $strings['pro-version-text%s'];
		$install   = $strings['btn-license-activate'];
		?>
		<div class="merlin__content--transition">
			<img src="<?php echo esc_url( BLOCKST_URI . '/assets/admin/img/theme_logo.png' ); ?>" class="merlin__content--pro-version-img" alt="<?php echo esc_attr( $theme ); ?>">
			<h1><?php echo esc_html( sprintf( $header, $theme ) ); ?></h1>
			<p><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>
			<div class="merlin__content--pro-version">
				<ul class="merlin__content--feature-list">
					<?php	if ( is_array( $features ) ) : ?>
						<?php foreach ( $features as $feature ) : ?>
							<li>
								<i class="merlin__content--feature-icon dashicons dashicons-yes" aria-hidden="true"></i>
								<a href="<?php echo esc_url( $feature['url'] ); ?>" target="_blank" rel="noopener"><?php echo esc_html( $feature['title'] ); ?></a>
							</li>
						<?php endforeach; ?>
					<?php endif; ?>
				</ul>
		...
		<footer class="merlin__content__footer">
			...
			<a href="<?php echo esc_url( blockst_fs()->get_upgrade_url() ); ?>" class="merlin__button merlin__button--next merlin__button--pro-version">
				<span class="merlin__button--loading__text"><?php echo esc_html( $install ); ?></span>
			</a>
			<?php wp_nonce_field( 'merlin' ); ?>
		</footer>
		<?php
		$this->logger->debug( __( 'The pro version step has been displayed', 'blockst' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'blockst' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'blockst' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'blockst' ); ?>">
										<?php esc_html_e( 'req', 'blockst' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'blockst' ) );
	}
		...
		<div class="merlin__content--transition">
			...
			<?php
				if ( 1 < count( $this->import_files ) ) {
					...
					printf( "<img src='%s' alt='" . esc_attr__( 'Home Preview', 'blockst' ) . "'class='merlin__image-preview' />", $this->import_files[0]['import_preview_image_url'] );
				}
			?>
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'blockst' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'blockst' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'blockst' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'blockst' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'blockst' ) );
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'blockst' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'blockst' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'blockst' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'blockst' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'blockst' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'blockst' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'blockst' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'blockst' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'blockst' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'blockst' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'blockst' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'blockst' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'blockst' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'blockst' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'blockst' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'blockst' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'blockst' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'blockst' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				...
				'description'      => esc_html__( 'Demo content data.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'blockst' ),
				'description'      => esc_html__( 'Sample widgets data.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'blockst' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'blockst' ),
				'description'      => esc_html__( 'Sample theme options data.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'blockst' ),
				'description'      => esc_html__( 'Redux framework options.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'blockst' ),
				'description'      => esc_html__( 'After import setup.', 'blockst' ),
				'pending'          => esc_html__( 'Pending', 'blockst' ),
				'installing'       => esc_html__( 'Installing', 'blockst' ),
				'success'          => esc_html__( 'Success', 'blockst' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'blockst' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'blockst' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( __( 'The home page was set', 'blockst' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'blockst' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'blockst' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'blockst' ), $import_file );
			}
		}
